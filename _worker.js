...
`;
    ...
    if (!userAgent.includes("CF-Workers-SUB".toLowerCase())) {
      if (
        ...
        url = `https://${subconverter}/sub?target=clash&url=${encodeURIComponent(
          url
        )}&insert=false&config=${encodeURIComponent(
          subconfig
        )}&tls13=true&emoji=true&list=false&xudp=false&udp=false&tfo=true&expand=false&scv=true&fdn=false&new_name=true`;
        isBase64 = false;
      } else if (
        ...
        url = `https://${subconverter}/sub?target=singbox&url=${encodeURIComponent(
          url
        )}&insert=false&config=${encodeURIComponent(
          subconfig
        )}&tls13=true&emoji=true&list=false&xudp=false&udp=false&tfo=true&expand=false&scv=true&fdn=false&new_name=true`;
  ...
  if (noTLS == "true") {
    ...
    notlsresponseBody = uniqueAddressesnotls
      .map((address) => {
        ...
        const vlessLink = `vless://${UUID}@${address}:${port}?encryption=none&security=&type=ws&host=${伪装域名}&path=${encodeURIComponent(
          最终路径
