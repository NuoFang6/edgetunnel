  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (proxyIP.includes(","))
        proxyIP =
          proxyIP.split(",")[
            Math.floor(Math.random() * proxyIP.split(",").length)
          ];
...
async function getVLESSConfig(userID, hostName, sub, userAgent, RproxyIP) {
  ...
  if (!sub || sub === "") {
    ...
    if (userAgent.includes("clash") && !userAgent.includes("nekobox")) {
      url = `https://${subconverter}/sub?target=clash&url=https%3A%2F%2F${sub}%2Fsub%3Fhost%3D${fakeHostName}%26uuid%3D${fakeUserID}%26edgetunnel%3Dcmliu%26proxyip%3D${RproxyIP}&insert=false&config=${encodeURIComponent(
        subconfig
      ...
      url = `https://${subconverter}/sub?target=singbox&url=https%3A%2F%2F${sub}%2Fsub%3Fhost%3D${fakeHostName}%26uuid%3D${fakeUserID}%26edgetunnel%3Dcmliu%26proxyip%3D${RproxyIP}&insert=false&config=${encodeURIComponent(
        subconfig
      )}&emoji=true&list=false&tfo=false&scv=true&fdn=false&sort=false&new_name=true`;
    } else {
