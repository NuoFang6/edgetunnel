...
let userID = 'ad9deb10-ee04-4b94-a02f-0cb834753a4d';
let proxyIP = 'usa.cmbaby.mom';
let sub = 'VLESS.fxxk.dedyn.io';
let subConverter = 'url.v1.mk';
let subConfig = "https://github.com/NuoFang6/nothing/raw/main/ACL4SSR/ACL4SSR.ini";
		...
		try {
			...
			if (!upgradeHeader || upgradeHeader !== 'websocket') {
				...
				if (路径 == '/') {
					...
					const qpdkfjcyzoConfig = await 生成配置信息(userID, request.headers.get('Host'), sub, UA, RproxyIP, url, env);
					const now = Date.now();
					...
					if (userAgent && userAgent.includes('mozilla')) {
						return new Response(`${qpdkfjcyzoConfig}`, {
							status: 200,
							headers: {
				...
				return await qpdkfjcyzoOverWSHandler(request);
			}
		} catch (err) {
			let e = err;
...
async function qpdkfjcyzoOverWSHandler(request) {
	...
	readableWebSocketStream.pipeTo(new WritableStream({
		async write(chunk, controller) {
			...
			const {
				...
				addressRemote = '',
				rawDataIndex,
				qpdkfjcyzoVersion = new Uint8Array([0, 0]),
				isUDP,
			} = processqpdkfjcyzoHeader(chunk, userID);
			// 设置地址和端口信息，用于日志
			...
			// 构建 qpdkfjcyzo 响应头部
			const qpdkfjcyzoResponseHeader = new Uint8Array([qpdkfjcyzoVersion[0], 0]);
			// 获取实际的客户端数据
			const rawClientData = chunk.slice(rawDataIndex);

			if (isDns) {
				// 如果是 DNS 查询，调用 DNS 处理函数
				return handleDNSQuery(rawClientData, webSocket, qpdkfjcyzoResponseHeader, log);
			}
			// 处理 TCP 出站连接
			log(`处理 TCP 出站连接 ${addressRemote}:${portRemote}`);
			handleTCPOutBound(remoteSocketWapper, addressType, addressRemote, portRemote, rawClientData, webSocket, qpdkfjcyzoResponseHeader, log);
		},
		close() {
...
async function handleTCPOutBound(remoteSocket, addressType, addressRemote, portRemote, rawClientData, webSocket, qpdkfjcyzoResponseHeader, log,) {
	...
	async function retry() {
		...
		remoteSocketToWS(tcpSocket, webSocket, qpdkfjcyzoResponseHeader, null, log);
	}
	...
	remoteSocketToWS(tcpSocket, webSocket, qpdkfjcyzoResponseHeader, retry, log);
}
...
// https://xtls.github.io/development/protocols/qpdkfjcyzo.html
// https://github.com/zizifn/excalidraw-backup/blob/main/v2ray-protocol.excalidraw

/**
 * 解析 qpdkfjcyzo 协议的头部数据
 * @param { ArrayBuffer} qpdkfjcyzoBuffer qpdkfjcyzo 协议的原始头部数据
 * @param {string} userID 用于验证的用户 ID
 * @returns {Object} 解析结果，包括是否有错误、错误信息、远程地址信息等
 */
function processqpdkfjcyzoHeader(qpdkfjcyzoBuffer, userID) {
	// 检查数据长度是否足够（至少需要 24 字节）
	if (qpdkfjcyzoBuffer.byteLength < 24) {
		return {
			hasError: true,
	...
	// 解析 qpdkfjcyzo 协议版本（第一个字节）
	const version = new Uint8Array(qpdkfjcyzoBuffer.slice(0, 1));
	...
	// 使用函数验证
	isValidUser = isUserIDValid(userID, userIDLow, qpdkfjcyzoBuffer);

	// 如果用户 ID 无效，返回错误
	if (!isValidUser) {
		return {
			hasError: true,
			message: `invalid user ${(new Uint8Array(qpdkfjcyzoBuffer.slice(1, 17)))}`,
		};
	}

	// 获取附加选项的长度（第 17 个字节）
	const optLength = new Uint8Array(qpdkfjcyzoBuffer.slice(17, 18))[0];
	// 暂时跳过附加选项

	// 解析命令（紧跟在选项之后的 1 个字节）
	// 0x01: TCP, 0x02: UDP, 0x03: MUX（多路复用）
	const command = new Uint8Array(
		qpdkfjcyzoBuffer.slice(18 + optLength, 18 + optLength + 1)
	)[0];
	...
	const portBuffer = qpdkfjcyzoBuffer.slice(portIndex, portIndex + 2);
	...
	const addressBuffer = new Uint8Array(
		qpdkfjcyzoBuffer.slice(addressIndex, addressIndex + 1)
	);
	...
	switch (addressType) {
		case 1:
			...
			addressValue = new Uint8Array(
				qpdkfjcyzoBuffer.slice(addressValueIndex, addressValueIndex + addressLength)
			).join('.');
			break;
		case 2:
			// 域名
			...
			addressLength = new Uint8Array(
				qpdkfjcyzoBuffer.slice(addressValueIndex, addressValueIndex + 1)
			)[0];
			addressValueIndex += 1;
			// 解码域名
	...
	return {
		...
		qpdkfjcyzoVersion: version,	  // qpdkfjcyzo 协议版本
		isUDP,					 // 是否是 UDP 请求
	};
}

async function remoteSocketToWS(remoteSocket, webSocket, qpdkfjcyzoResponseHeader, retry, log) {
	// 将数据从远程服务器转发到 WebSocket
	let remoteChunkCount = 0;
	let chunks = [];
	/** @type {ArrayBuffer | null} */
	let qpdkfjcyzoHeader = qpdkfjcyzoResponseHeader;
		...
		.pipeTo(
			new WritableStream({
				...
				async write(chunk, controller) {
					...
					if (qpdkfjcyzoHeader) {
						// 如果有 qpdkfjcyzo 响应头部，将其与第一个数据块一起发送
						webSocket.send(await new Blob([qpdkfjcyzoHeader, chunk]).arrayBuffer());
						qpdkfjcyzoHeader = null; // 清空头部，之后不再发送
					} else {
						// 直接发送数据块
 ...
 * @param {ArrayBuffer} qpdkfjcyzoResponseHeader - qpdkfjcyzo 协议的响应头部数据
 * @param {(string)=> void} log - 日志记录函数
 */
async function handleDNSQuery(udpChunk, webSocket, qpdkfjcyzoResponseHeader, log) {
	...
	try {
		...
		let qpdkfjcyzoHeader = qpdkfjcyzoResponseHeader; // 保存 qpdkfjcyzo 响应头部，用于后续发送
		...
		await tcpSocket.readable.pipeTo(new WritableStream({
			async write(chunk) {
				if (webSocket.readyState === WS_READY_STATE_OPEN) {
					if (qpdkfjcyzoHeader) {
						// 如果有 qpdkfjcyzo 头部，则将其与 DNS 响应数据合并后发送
						webSocket.send(await new Blob([qpdkfjcyzoHeader, chunk]).arrayBuffer());
						qpdkfjcyzoHeader = null; // 头部只发送一次，之后置为 null
					} else {
						// 否则直接发送 DNS 响应数据
						webSocket.send(chunk);
					}
...
function 生成本地订阅(host, UUID, noTLS, newAddressesapi, newAddressescsv, newAddressesnotlsapi, newAddressesnotlscsv) {
	...
	if (noTLS == 'true') {
		...
		notlsresponseBody = uniqueAddressesnotls.map(address => {
			...
			const qpdkfjcyzoLink = `${协议类型}://${UUID}@${address}:${port + atob('P2VuY3J5cHRpb249bm9uZSZzZWN1cml0eT0mdHlwZT13cyZob3N0PQ==') + 伪装域名}&path=${encodeURIComponent(最终路径)}#${encodeURIComponent(addressid + 节点备注)}`;

			return qpdkfjcyzoLink;
	...
	const responseBody = uniqueAddresses.map(address => {
		...
		const 协议类型 = atob(啥啥啥_写的这是啥啊);
		const qpdkfjcyzoLink = `${协议类型}://${UUID}@${address}:${port + atob('P2VuY3J5cHRpb249bm9uZSZzZWN1cml0eT10bHMmc25pPQ==') + 伪装域名}&fp=random&type=ws&host=${伪装域名}&path=${encodeURIComponent(最终路径)}#${encodeURIComponent(addressid + 节点备注)}`;
